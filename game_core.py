...
class GameState:
    ...
    # ========== Evaluation Helpers for AI Heuristics ==========
    ...
    def get_smoothness(self) -> float:
        ...
        Example:
            ...
            [[2, 512, 2, 512],
             [512, 2, 512, 2],  -> High smoothness (very different neighbors)
             [2, 512, 2, 512],
             [512, 2, 512, 2]]
        """
        smoothness = 0.0
            ...
            for j in range(4):
                if self.grid[i, j] != 0:
                    ...
                    if j < 3 and self.grid[i, j+1] != 0:
                        smoothness += abs(value - self.grid[i, j+1])
                    
                    # Check down neighbor
                    if i < 3 and self.grid[i+1, j] != 0:
                        smoothness += abs(value - self.grid[i+1, j])
        
        return smoothness
    
    def get_monotonicity(self) -> float:
        """
        ...
        # Check rows
        for i in range(4):
            score += monotonic_score(self.grid[i, :])
        
        # Check columns
        for j in range(4):
            score += monotonic_score(self.grid[:, j])
        
        return score
    
    def get_merge_potential(self) -> int:
        """
        ...
        for i in range(4):
            for j in range(4):
                if self.grid[i, j] != 0:
                    ...
                    # Check right neighbor
                    if j < 3 and self.grid[i, j+1] == value:
                        merges += 1
                    
                    # Check down neighbor
                    if i < 3 and self.grid[i+1, j] == value:
                        merges += 1
        
        return merges
    
    def get_corner_weight(self) -> float:
        """
        ...
        for i, j in corners:
            weight += self.grid[i, j] * 4.0
        
        # Rest have 1x weight
        for i in range(4):
            for j in range(4):
                if (i, j) not in corners and self.grid[i, j] != 0:
                    weight += self.grid[i, j] * 1.0
        
        return weight
    
    def get_position_weights(self) -> float:
        """
        ...
        return float(np.sum(self.grid * weights))
    
    def count_distinct_tiles(self) -> int:
        """
    ...
    def is_symmetric(self) -> bool:
        ...
        h_sym = np.array_equal(self.grid, np.flip(self.grid, axis=0))
        
        # Check vertical symmetry
        v_sym = np.array_equal(self.grid, np.flip(self.grid, axis=1))
